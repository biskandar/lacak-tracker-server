...
public class HemisphereHandler extends ExtendedObjectDecoder {
  ...
  public HemisphereHandler() {
    String latitudeHemisphere = Context.getConfig().getString(
    ...
    String longitudeHemisphere = Context.getConfig().getString(
  ...
  protected Object decode(Channel channel, SocketAddress remoteAddress,
    ...
    if (msg instanceof Position) {
      ...
      if (longitudeFactor != 0) {
        position.setLongitude(Math.abs(position.getLongitude())
            * longitudeFactor);
      }
    }
