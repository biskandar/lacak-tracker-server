...
public class FilterHandler extends BaseDataHandler {
  ...
  public FilterHandler(boolean filterInvalid, boolean filterZero,
      boolean filterDuplicate, boolean filterFuture, boolean filterApproximate,
      int filterDistance, long filterLimit) {
  ...
  private boolean filterZero(Position position) {
    return filterZero && position.getLatitude() == 0.0
  ...
  private boolean filterFuture(Position position) {
    return filterFuture
        && position.getFixTime().getTime() > System.currentTimeMillis()
            + FILTER_FUTURE_LIMIT;
  }
  
  private boolean filterApproximate(Position position) {
    Boolean approximate = (Boolean) position.getAttributes().get(
        Position.KEY_APPROXIMATE);
  ...
  private boolean filterDistance(Position position) {
    if (filterDistance != 0) {
      Position last = getLastPosition(position.getDeviceId());
      if (last != null) {
        double distance = DistanceCalculator.distance(position.getLatitude(),
            position.getLongitude(), last.getLatitude(), last.getLongitude());
