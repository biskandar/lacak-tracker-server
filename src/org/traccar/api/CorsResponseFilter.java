...
public class CorsResponseFilter implements ContainerResponseFilter {
  ...
  public void filter(ContainerRequestContext request,
    ...
    if (!response.getHeaders().containsKey(
      ...
      response.getHeaders().add(HttpHeaders.Names.ACCESS_CONTROL_ALLOW_HEADERS,
          HEADERS_ALL);
    }
    ...
    if (!response.getHeaders().containsKey(
      ...
      response.getHeaders().add(
          HttpHeaders.Names.ACCESS_CONTROL_ALLOW_CREDENTIALS, true);
    }
    
    if (!response.getHeaders().containsKey(
        HttpHeaders.Names.ACCESS_CONTROL_ALLOW_METHODS)) {
      response.getHeaders().add(HttpHeaders.Names.ACCESS_CONTROL_ALLOW_METHODS,
          METHODS_ALL);
    }
    ...
    if (!response.getHeaders().containsKey(
      ...
      if (origin == null) {
        response.getHeaders().add(
            HttpHeaders.Names.ACCESS_CONTROL_ALLOW_ORIGIN, ORIGIN_ALL);
      } else if (allowed == null || allowed.equals(ORIGIN_ALL)
