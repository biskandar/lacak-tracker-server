...
public class TrvProtocolDecoder extends BaseProtocolDecoder {
  ...
  private static final Pattern PATTERN = new PatternBuilder().text("TRV")
      .number("APdd").number("(dd)(dd)(dd)") // date
      .expression("([AV])") // validity
      .number("(dd)(dd.d+)") // latitude
      .expression("([NS])").number("(ddd)(dd.d+)") // longitude
      .expression("([EW])").number("(ddd.d)") // speed
  ...
  private static final Pattern PATTERN_HEATRBEAT = new PatternBuilder()
      .text("TRV").text("CP01,").number("(ddd)") // gsm
      .number("(ddd)") // gps
      .number("(ddd)") // battery
      .number("(d)") // acc
      .number("(dd)") // arm status
  ...
  protected Object decode(Channel channel, SocketAddress remoteAddress,
    ...
    if (channel != null) {
      channel.write("TRV" + (char) (type.charAt(0) + 1) + type.substring(1)
          + "#"); // response
    }
