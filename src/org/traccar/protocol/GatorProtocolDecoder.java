...
public class GatorProtocolDecoder extends BaseProtocolDecoder {
  ...
  public static String decodeId(int b1, int b2, int b3, int b4) {
    ...
    int d1 = 30 + ((b1 >> 7) << 3) + ((b2 >> 7) << 2) + ((b3 >> 7) << 1)
        + (b4 >> 7);
    int d2 = b1 & 0x7f;
    int d3 = b2 & 0x7f;
    int d4 = b3 & 0x7f;
  ...
  protected Object decode(Channel channel, SocketAddress remoteAddress,
    ...
    String id = decodeId(buf.readUnsignedByte(), buf.readUnsignedByte(),
    ...
    if (type == MSG_HEARTBEAT) {
      ...
      position
          .setSpeed(UnitsConverter.knotsFromKph(BcdUtil.readInteger(buf, 4)));
      ...
      position.set(Position.KEY_POWER,
          buf.readUnsignedByte() + buf.readUnsignedByte() / 100.0);
