...
public class SuntechProtocolDecoder extends BaseProtocolDecoder {
  ...
  private static final Pattern PATTERN = new PatternBuilder().expression("S.")
      .number("ddd").expression("(?:[A-Z]{3})?;") // header
      .expression("([^;]+);").optional() // type
      .number("(d{6,});") // device id
      .number("d+;").optional().number("(d+);") // version
      ...
      .number("d+;").optional().number("(d+.d+)?") // battery
      .any() // full format
      .compile();
