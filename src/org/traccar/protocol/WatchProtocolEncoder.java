...
public class WatchProtocolEncoder extends StringProtocolEncoder {
  ...
  protected String formatCommand(Command command, String format, String... keys) {
    ...
    return String.format("[CS*%s*%04x*%s]", getUniqueId(command.getDeviceId()),
  ...
  private String getBinaryData(Command command) {
    byte[] data = DatatypeConverter.parseHexBinary((String) command
  ...
  protected Object encodeCommand(Command command) {
    ...
    case Command.TYPE_SOS_NUMBER:
      return formatCommand(command, "SOS{%s},{%s}", Command.KEY_INDEX,
          Command.KEY_PHONE);
