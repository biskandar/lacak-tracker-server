...
public class GranitProtocolDecoder extends BaseProtocolDecoder {
  ...
  private static void sendResponseCurrent(Channel channel, int deviceId,
      long time) {
  ...
  private static void sendResponseArchive(Channel channel, int deviceId,
      int packNum) {
    ChannelBuffer response = ChannelBuffers.dynamicBuffer(
        ByteOrder.LITTLE_ENDIAN, 0);
  ...
  protected Object decode(Channel channel, SocketAddress remoteAddress,
    ...
    int indexTilde = buf.indexOf(buf.readerIndex(), buf.writerIndex(),
        new StringFinder("~"));
    ...
    String header = buf.readBytes(HEADER_LENGTH).toString(
