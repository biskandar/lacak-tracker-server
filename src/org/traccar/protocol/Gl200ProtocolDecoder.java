...
public class Gl200ProtocolDecoder extends BaseProtocolDecoder {
  ...
  private static final Pattern PATTERN_HBD = new PatternBuilder()
      .text("+ACK:GTHBD,").number("([0-9A-Z]{2}xxxx),").any().text(",")
      .number("(xxxx)").text("$").optional().compile();
  
  private static final Pattern PATTERN_INF = new PatternBuilder()
      .text("+RESP:GTINF,").number("[0-9A-Z]{2}xxxx,")
      // protocol version
      .number("(d{15}),")
      // imei
      ...
      .expression("[0-9F]{20},")
      // iccid
      .number("d{1,2},").number("d{1,2},").expression("[01],")
      .number("(d{1,5}),") // power
      .text(",").number("(d+.d+),") // battery
      .expression("([01]),") // charging
      .expression("[01],").text(",,").number("d{14},") // last fix time
      .text(",,,,,").number("[-+]dddd,") // timezone
  ...
  private static final Pattern PATTERN_LOCATION = new PatternBuilder()
      ...
      .text(",").groupBegin().number("(0ddd)?,") // mcc
      .number("(0ddd)?,") // mnc
      .number("(xxxx)?,") // lac
      .number("(xxxx)?,") // cell
      .or().number("(d+)?,") // mcc
      ...
      .number("(d+)?,") // lac
      .number("(d+)?,") // cell
      .groupEnd().number("(?:d+|(d+.d))?,") // odometer
      .compile();
  
  private static final Pattern PATTERN_OBD = new PatternBuilder()
      .text("+RESP:GTOBD,").number("[0-9A-Z]{2}xxxx,") // protocol version
      .number("(d{15}),") // imei
      ...
      .number("(?:d{1,5})?,").expression("([01])?,") // obd connect
      ...
      .number("(d+),") // odometer
      .expression(PATTERN_LOCATION.pattern()).number("(d{1,7}.d)?,") // odometer
      .number("(dddd)(dd)(dd)") // date
      .number("(dd)(dd)(dd)").optional(2) // time
      .text(",").number("(xxxx)") // count number
      .text("$").optional().compile();
  
  private static final Pattern PATTERN_FRI = new PatternBuilder().text("+")
      .expression("(?:RESP|BUFF):GTFRI,").number("(?:[0-9A-Z]{2}xxxx)?,")
      // protocol version
      .number("(d{15}|x{14}),")
      // imei
      .expression("(?:([0-9A-Z]{17}),)?")
      ...
      .number("d{1,2},")
      // count
      .expression("((?:").expression(PATTERN_LOCATION.pattern())
      .expression(")+)").groupBegin().number("(d{1,7}.d)?,").optional() // odometer
      .number("(d{1,3})?,") // battery
      .or().number("(d{1,7}.d)?,") // odometer
      ...
      .number("(?:d+.?d*|Inf|NaN)?,") // fuel consumption
      .number("(d+)?,") // fuel level
      .groupEnd().number("(dddd)(dd)(dd)") // date
      .number("(dd)(dd)(dd)").optional(2) // time
      .text(",").number("(xxxx)") // count number
      .text("$").optional().compile();
  
  private static final Pattern PATTERN_IGN = new PatternBuilder().text("+")
      .expression("(?:RESP|BUFF):GTIG[NF],").number("(?:[0-9A-Z]{2}xxxx)?,") // protocol
                                                                             // version
      .number("(d{15}),") // imei
      .expression("[^,]*,") // device name
      .number("d+,") // ignition off duration
      .expression(PATTERN_LOCATION.pattern()).number("(d{5}:dd:dd)?,") // hour
                                                                       // meter
  ...
  private static final Pattern PATTERN_IDA = new PatternBuilder()
      .text("+RESP:GTIDA,").number("(?:[0-9A-Z]{2}xxxx)?,") // protocol version
      .number("(d{15}),") // imei
      .expression("[^,]*,,") // device name
      .number("([^,]+),") // rfid
      .expression("[01],") // report type
      .number("1,") // count
      .expression(PATTERN_LOCATION.pattern()).number("(d+.d),") // odometer
      .text(",,,,").number("(dddd)(dd)(dd)") // date
      .number("(dd)(dd)(dd)").optional(2) // time
      .text(",").number("(xxxx)") // count number
      .text("$").optional().compile();
  
  private static final Pattern PATTERN = new PatternBuilder().text("+")
      .expression("(?:RESP|BUFF):GT...,").number("(?:[0-9A-Z]{2}xxxx)?,")
      // protocol version
      .number("(d{15}|x{14}),")
      // imei
      .expression("[^,]*,")
      // device name
      .number("d*,").number("(d{1,2}),")
      // report type
      .number("d{1,2},")
      // count
      .expression(PATTERN_LOCATION.pattern()).groupBegin()
  ...
  private static final Pattern PATTERN_BASIC = new PatternBuilder().text("+")
      .expression("(?:RESP|BUFF)").text(":").expression("GT...,")
      .number("(?:[0-9A-Z]{2}xxxx)?,") // protocol version
      .number("(d{15}|x{14}),") // imei
      .any().number("(d{1,3}.d)?,") // speed
      .number("(d{1,3})?,") // course
      .number("(-?d{1,5}.d)?,") // altitude
      .number("(-?d{1,3}.d{6}),") // longitude
      .number("(-?d{1,2}.d{6}),") // latitude
      .number("(dddd)(dd)(dd)") // date
      .number("(dd)(dd)(dd)") // time
      .text(",").any().number("(dddd)(dd)(dd)") // date
      .number("(dd)(dd)(dd)").optional(2) // time
      .text(",").number("(xxxx)") // count number
      .text("$").optional().compile();
  
  private Object decodeHbd(Channel channel, SocketAddress remoteAddress,
      String sentence) {
    Parser parser = new Parser(PATTERN_HBD, sentence);
    if (parser.matches() && channel != null) {
      channel.write("+SACK:GTHBD," + parser.next() + "," + parser.next() + "$",
          remoteAddress);
    }
    return null;
  }
  
  private Object decodeInf(Channel channel, SocketAddress remoteAddress,
      String sentence) {
  ...
  private Object decodeObd(Channel channel, SocketAddress remoteAddress,
      String sentence) {
    ...
    if (parser.hasNext(6)) {
      ...
      if (!position.getOutdated()
          && position.getFixTime().after(dateBuilder.getDate())) {
        position.setTime(dateBuilder.getDate());
      }
    }
  ...
  private Object decodeFri(Channel channel, SocketAddress remoteAddress,
      String sentence) {
  ...
  private Object decodeIgn(Channel channel, SocketAddress remoteAddress,
      String sentence) {
  ...
  private Object decodeIda(Channel channel, SocketAddress remoteAddress,
      String sentence) {
  ...
  private Object decodeOther(Channel channel, SocketAddress remoteAddress,
      String sentence, String type) {
    ...
    if (Context.getConfig().getBoolean(getProtocolName() + ".ack")
        && channel != null) {
  ...
  private Object decodeBasic(Channel channel, SocketAddress remoteAddress,
      String sentence) {
    ...
    if (parser.hasNext(6)) {
      dateBuilder = new DateBuilder().setDate(parser.nextInt(),
          parser.nextInt(), parser.nextInt()).setTime(parser.nextInt(),
          parser.nextInt(), parser.nextInt());
      if (!position.getOutdated()
          && position.getFixTime().after(dateBuilder.getDate())) {
