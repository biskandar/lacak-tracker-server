...
public class MeitrackProtocolEncoder extends StringProtocolEncoder {
  ...
  private Object formatCommand(Command command, char dataId, String content) {
    ...
    String result = String.format("@@%c%02d,%s,%s*", dataId, length, uniqueId,
        content);
  ...
  protected Object encodeCommand(Command command) {
    ...
    case Command.TYPE_SEND_SMS:
      return formatCommand(command, 'f',
          "C02,0," + attributes.get(Command.KEY_PHONE_NUMBER) + ","
              + attributes.get(Command.KEY_MESSAGE));
    default:
