...
public class EnforaProtocolDecoder extends BaseProtocolDecoder {
  ...
  private static final Pattern PATTERN = new PatternBuilder().text("GPRMC,")
  ...
  protected Object decode(Channel channel, SocketAddress remoteAddress,
    ...
    // Find IMEI number
    int index = buf.indexOf(buf.readerIndex(), buf.writerIndex(),
    ...
    // Find NMEA sentence
    int start = buf.indexOf(buf.readerIndex(), buf.writerIndex(),
        new StringFinder("GPRMC"));
    if (start == -1) {
      return null;
    }
    
    String sentence = buf.toString(start, buf.readableBytes() - start,
