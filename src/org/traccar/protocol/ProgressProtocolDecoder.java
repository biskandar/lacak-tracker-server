...
public class ProgressProtocolDecoder extends BaseProtocolDecoder {
  ...
  private void requestArchive(Channel channel) {
    if (lastIndex == 0) {
      ...
      ChannelBuffer request = ChannelBuffers.directBuffer(
          ByteOrder.LITTLE_ENDIAN, 12);
  ...
  protected Object decode(Channel channel, SocketAddress remoteAddress,
    ...
    if (type == MSG_IDENT || type == MSG_IDENT_FULL) {
      ...
      identify(imei, channel, remoteAddress);
        ...
        && (type == MSG_POINT || type == MSG_ALARM || type == MSG_LOGMSG)) {
      ...
      for (int j = 0; j < recordCount; j++) {
        ...
        if (BitUtil.check(extraFlags, 1)) {
          ...
          position.set("can",
              buf.toString(buf.readerIndex(), size, StandardCharsets.US_ASCII));
          buf.skipBytes(size);
        }
        ...
        if (type == MSG_ALARM) {
          position.set(Position.KEY_ALARM, true);
          byte[] response = { (byte) 0xC9, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
              0x00 };
