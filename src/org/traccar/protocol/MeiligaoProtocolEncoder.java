...
public class MeiligaoProtocolEncoder extends BaseProtocolEncoder {
  ...
  private ChannelBuffer encodeContent(long deviceId, int type,
    ...
    buf.writeShort(2 + 2 + 7 + 2 + content.readableBytes() + 2 + 2); // message
                                                                     // length
    
    buf.writeBytes(DatatypeConverter
        .parseHexBinary((getUniqueId(deviceId) + "FFFFFFFFFFFFFF").substring(0,
            14)));
  ...
  protected Object encodeCommand(Command command) {
    ...
    case Command.TYPE_POSITION_PERIODIC:
      content.writeShort(((Number) attributes.get(Command.KEY_FREQUENCY))
          .intValue() / 10);
      return encodeContent(command.getDeviceId(), MSG_TRACK_BY_INTERVAL,
          content);
    ...
    case Command.TYPE_ALARM_GEOFENCE:
      content.writeShort(((Number) attributes.get(Command.KEY_RADIUS))
          .intValue());
    ...
    case Command.TYPE_SET_TIMEZONE:
      ...
      content.writeBytes(String.valueOf(offset).getBytes(
