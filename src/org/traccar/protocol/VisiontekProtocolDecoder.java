...
public class VisiontekProtocolDecoder extends BaseProtocolDecoder {
  ...
  private static final Pattern PATTERN = new PatternBuilder().text("$1,")
      .expression("([^,]+),") // identifier
      .number("(d+),").optional() // imei
      .number("(dd),(dd),(dd),") // date
      .number("(dd),(dd),(dd),") // time
      .groupBegin().number("(dd)(dd).?(d+)([NS]),") // latitude
      .number("(ddd)(dd).?(d+)([EW]),") // longitude
      .or().number("(dd.d+)([NS]),") // latitude
      .number("(ddd.d+)([EW]),") // longitude
      .groupEnd().number("(d+.?d+),") // speed
      .number("(d+),") // course
      .groupBegin().groupBegin().number("(d+),") // altitude
      .number("(d+),") // satellites
      .groupEnd("?").number("(d+),") // odometer
      .groupBegin().number("(d),") // ignition
      .number("(d),") // input 1
      .number("(d),") // input 2
      .number("(d),") // immobilizer
      .number("(d),") // external battery status
      .number("(d+),") // gsm
      .groupEnd("?").groupEnd("?").any().expression("([AV])") // validity
  ...
  protected Object decode(Channel channel, SocketAddress remoteAddress,
    ...
    if (!identify(id, channel, remoteAddress, false)
        && (imei == null || !identify(imei, channel, remoteAddress))) {
      return null;
    }
    ...
    if (parser.hasNext(8)) {
      position.setLatitude(parser
          .nextCoordinate(Parser.CoordinateFormat.DEG_MIN_MIN_HEM));
    ...
    position.setSpeed(UnitsConverter.knotsFromKph(Double.parseDouble(parser
        .next().replace(".", "")) / 10));
