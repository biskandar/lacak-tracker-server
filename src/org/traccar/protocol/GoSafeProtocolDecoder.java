...
public class GoSafeProtocolDecoder extends BaseProtocolDecoder {
  ...
  private static final Pattern PATTERN = new PatternBuilder().text("*GS") // header
  ...
  private static final Pattern PATTERN_ITEM = new PatternBuilder()
      ...
      .number("x+,?")
      // event status
      .groupEnd("?").groupBegin().text("ETD:").expression("[^,]*,?")
      .groupEnd("?").groupBegin().text("OBD:").number("(x+),?").groupEnd("?")
      .groupBegin().text("FUL:").expression("[^,]*,?").groupEnd("?")
      .groupBegin().text("TRU:").expression("[^,]*,?").groupEnd("?").compile();
  
  private static final Pattern PATTERN_OLD = new PatternBuilder().text("*GS") // header
      .number("d+,") // protocol version
      .number("(d+),") // imei
      .text("GPS:").number("(dd)(dd)(dd);") // time
  ...
  private Position decodePosition(Parser parser, Date time) {
    ...
    position
        .setLatitude(parser.nextCoordinate(Parser.CoordinateFormat.HEM_DEG));
    position.setLongitude(parser
        .nextCoordinate(Parser.CoordinateFormat.HEM_DEG));
    ...
    if (status != null) {
      position.set(Position.KEY_IGNITION,
          BitUtil.check(Integer.parseInt(status, 16), 13));
  ...
  protected Object decode(Channel channel, SocketAddress remoteAddress,
    ...
    if (pattern == PATTERN_OLD) {
      ...
      if (parser.hasNext(6)) {
        DateBuilder dateBuilder = new DateBuilder().setTime(parser.nextInt(),
            parser.nextInt(), parser.nextInt()).setDateReverse(
