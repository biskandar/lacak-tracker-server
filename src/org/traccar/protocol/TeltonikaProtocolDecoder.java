...
public class TeltonikaProtocolDecoder extends BaseProtocolDecoder {
  ...
  private void parseIdentification(Channel channel,
      SocketAddress remoteAddress, ChannelBuffer buf) {
    
    int length = buf.readUnsignedShort();
    String imei = buf.toString(buf.readerIndex(), length,
  ...
  private void decodeSerial(Position position, ChannelBuffer buf) {
    ...
    position.set("command",
        buf.readBytes(buf.readInt()).toString(StandardCharsets.US_ASCII));
    ...
    if (codec == CODEC_GH3000) {
      ...
      if (BitUtil.check(globalMask, 0)) {
        ...
        if (BitUtil.check(locationMask, 3)) {
          position
              .setSpeed(UnitsConverter.knotsFromKph(buf.readUnsignedByte()));
        }
