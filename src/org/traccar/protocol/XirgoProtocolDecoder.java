...
public class XirgoProtocolDecoder extends BaseProtocolDecoder {
  ...
  private static final Pattern FIRMWARE = Pattern
      .compile("\\w{4}-([0-9A-F]{7})");
  
  private static final Pattern PATTERN_OLD = new PatternBuilder().text("$$")
      .number("(d+),") // imei
      .number("(d+),") // event
  ...
  private static final Pattern PATTERN_NEW = new PatternBuilder().text("$$")
      .number("(d+),") // imei
      .number("(d+),") // event
      .number("(dddd)/(dd)/(dd),") // date
      .number("(dd):(dd):(dd),") // time
  ...
  protected Object decode(Channel channel, SocketAddress remoteAddress,
    ...
    if (matcher.find()) {
      ...
      if (type.equals("1137CD1") || type.equals("1137CC1")
          || type.equals("1137CA3")) {
        newFormat = true;
      }
    }
