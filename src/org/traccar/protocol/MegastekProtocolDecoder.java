...
public class MegastekProtocolDecoder extends BaseProtocolDecoder {
  ...
  private static final Pattern PATTERN_ALTERNATIVE = new PatternBuilder()
      ...
      .number("(?:(d+),)?") // outputs
      .number("(d.?d*),") // adc 1
      .groupBegin().number("(d.dd),") // adc 2
      .number("(d.dd),") // adc 3
      .groupEnd("?").expression("([^;]+);") // alarm
      .any() // checksum
      .compile();
  ...
  private Position decodeOld(Channel channel, SocketAddress remoteAddress,
      String sentence) {
    ...
    if (simple) {
      ...
      if (parser.matches()) {
        ...
        if (!identify(parser.next(), channel, remoteAddress, false)
  ...
  private static final Pattern PATTERN_NEW = new PatternBuilder()
      .number("dddd").optional().text("$MGV").number("ddd,").number("(d+),") // imei
      .expression("[^,]*,") // name
      .expression("([RS]),").number("(dd)(dd)(dd),") // date (ddmmyy)
      .number("(dd)(dd)(dd),") // time
      .expression("([AV]),") // validity
      .number("(d+)(dd.d+),([NS]),") // latitude
      .number("(d+)(dd.d+),([EW]),") // longitude
      .number("dd,").number("(dd),") // satellites
      .number("dd,").number("(d+.d+),") // hdop
      .number("(d+.d+),") // speed
      .number("(d+.d+),") // course
      .number("(d+.d+),") // altitude
      .number("(d+.d+),") // odometer
      ...
      .number("(d+),") // adc2
      .number("(d+),") // adc3
      .groupBegin().number("(-?d+.?d*)") // temperature 1
      .or().text(" ").groupEnd().text(",").groupBegin().number("(-?d+.?d*)") // temperature
                                                                             // 2
      .or().text(" ").groupEnd().text(",").number("(d+)?,,") // rfid
      .number("(d+)?,") // battery
      .expression("([^,]*);") // alert
      .any().compile();
  
  private Position decodeNew(Channel channel, SocketAddress remoteAddress,
      String sentence) {
