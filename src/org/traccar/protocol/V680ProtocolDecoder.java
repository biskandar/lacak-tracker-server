...
public class V680ProtocolDecoder extends BaseProtocolDecoder {
  ...
  private static final Pattern PATTERN = new PatternBuilder().groupBegin()
      .number("#(d+)#") // imei
      .expression("([^#]*)#") // user
      .groupEnd("?").number("(d+)#") // fix
      .expression("([^#]+)#") // password
      .expression("([^#]+)#") // event
      .number("(d+)#") // packet number
      .expression("([^#]+)?#?") // gsm base station
      .expression("(?:[^#]+#)?").number("(d+)?(dd.d+),") // longitude
      .expression("([EW]),").number("(d+)?(dd.d+),") // latitude
      .expression("([NS]),").number("(d+.d+),") // speed
      .number("(d+.?d*)?#") // course
  ...
  protected Object decode(Channel channel, SocketAddress remoteAddress,
    ...
    if (sentence.length() == 16) {
      ...
      DateBuilder dateBuilder = new DateBuilder().setDate(parser.nextInt(),
          month, day).setTime(parser.nextInt(), parser.nextInt(),
          parser.nextInt());
