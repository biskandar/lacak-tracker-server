...
public class Mta6ProtocolDecoder extends BaseProtocolDecoder {
  ...
  private void sendResponse(Channel channel, short packetId, short packetCount) {
    ...
    ChannelBuffer begin = ChannelBuffers.copiedBuffer("#ACK#",
  ...
  private static class FloatReader {
    ...
    public float readFloat(ChannelBuffer buf) {
      ...
      case 0:
        ...
        break;
      case 1:
        previousFloat = (previousFloat & 0xffffff00)
            + ((buf.readUnsignedByte() & 0x3f) << 2);
        break;
      case 2:
        previousFloat = (previousFloat & 0xffff0000)
            + ((buf.readUnsignedShort() & 0x3fff) << 2);
        break;
      case 3:
        previousFloat = (previousFloat & 0xff000000)
            + ((buf.readUnsignedMedium() & 0x3fffff) << 2);
        break;
      default:
      ...
      while (buf.readable()) {
        ...
        if (BitUtil.check(flags, 4)) {
          position.set(Position.KEY_FUEL,
              buf.readUnsignedInt() + "|" + buf.readUnsignedInt());
        ...
        if (BitUtil.check(flags, 5)) {
          position.set(Position.PREFIX_ADC + 1,
              buf.readUnsignedShort() & 0x03ff);
          position.set(Position.PREFIX_ADC + 2,
              buf.readUnsignedShort() & 0x03ff);
          position.set(Position.PREFIX_ADC + 3,
              buf.readUnsignedShort() & 0x03ff);
          position.set(Position.PREFIX_ADC + 4,
              buf.readUnsignedShort() & 0x03ff);
        }
        ...
        if (BitUtil.check(flags, 7)) {
          ...
          position.set(Position.KEY_GSM,
              (buf.getUnsignedByte(buf.readerIndex()) >> 4) & 0x07);
  ...
  private Position parseFormatA1(ChannelBuffer buf) {
    ...
    if (BitUtil.check(flags, 5)) {
      ...
      position.set(Position.KEY_GSM,
          buf.getUnsignedByte(buf.readerIndex()) >> 5);
  ...
  protected Object decode(Channel channel, SocketAddress remoteAddress,
    ...
    String uniqueId = buf.toString(buf.readerIndex(),
        index - buf.readerIndex(), StandardCharsets.US_ASCII);
