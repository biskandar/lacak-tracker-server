...
public class FlextrackProtocolDecoder extends BaseProtocolDecoder {
  ...
  private static final Pattern PATTERN_LOGON = new PatternBuilder()
      .number("(-?d+),") // index
      .text("LOGON,").number("(d+),") // node id
      .number("(d+)") // iccid
      .compile();
  
  private static final Pattern PATTERN = new PatternBuilder().number("(-?d+),") // index
      .text("UNITSTAT,").number("(dddd)(dd)(dd),") // date
  ...
  protected Object decode(Channel channel, SocketAddress remoteAddress,
    ...
    if (sentence.contains("LOGON")) {
      ...
      if (!identify(iccid, channel, remoteAddress, false)
