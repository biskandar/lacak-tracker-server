...
public class SupermateProtocolDecoder extends BaseProtocolDecoder {
  ...
  private static final Pattern PATTERN = new PatternBuilder().number("d+:") // header
      .number("(d+):") // imei
      .number("d+:").text("*,").number("(d+),") // command id
      ...
      .number("(xx)") // minutes
      .number("(xx),") // seconds
      .number("(x)").number("(x{7}),") // latitude
      .number("(x)").number("(x{7}),") // longitude
      .number("(x{4}),") // speed
      .number("(x{4}),") // course
      .number("(x{12}),") // status
      .number("(x+),") // signal
  ...
  protected Object decode(Channel channel, SocketAddress remoteAddress,
    ...
    DateBuilder dateBuilder = new DateBuilder().setDate(parser.nextInt(16),
        parser.nextInt(16), parser.nextInt(16)).setTime(parser.nextInt(16),
    ...
    if (channel != null) {
      ...
      String content = String.format(
          "#1:%s:1:*,00000000,UP,%02x%02x%02x,%02x%02x%02x#", imei,
          calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH) + 1,
          calendar.get(Calendar.DAY_OF_MONTH),
          calendar.get(Calendar.HOUR_OF_DAY), calendar.get(Calendar.MINUTE),
          calendar.get(Calendar.SECOND));
      channel.write(ChannelBuffers.copiedBuffer(content,
