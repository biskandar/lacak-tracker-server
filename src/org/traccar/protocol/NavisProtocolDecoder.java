...
public class NavisProtocolDecoder extends BaseProtocolDecoder {
  ...
  private ParseResult parsePosition(ChannelBuffer buf) {
    ...
    if (isFormat(format, F20, F50, F51, F52)) {
      ...
      DateBuilder dateBuilder = new DateBuilder().setTime(
          ...
          buf.readUnsignedByte()).setDateReverse(buf.readUnsignedByte(),
          buf.readUnsignedByte(), buf.readUnsignedByte());
  ...
  private Object processHandshake(Channel channel, SocketAddress remoteAddress,
      ChannelBuffer buf) {
    buf.readByte(); // semicolon symbol
    if (identify(buf.toString(StandardCharsets.US_ASCII), channel,
        remoteAddress)) {
      sendReply(channel, ChannelBuffers.copiedBuffer(ByteOrder.LITTLE_ENDIAN,
          "*<S", StandardCharsets.US_ASCII));
    }
    return null;
  }
  ...
  private void sendReply(Channel channel, ChannelBuffer data) {
    ChannelBuffer header = ChannelBuffers.directBuffer(ByteOrder.LITTLE_ENDIAN,
        16);
    header.writeBytes(ChannelBuffers.copiedBuffer(ByteOrder.LITTLE_ENDIAN,
        prefix, StandardCharsets.US_ASCII));
