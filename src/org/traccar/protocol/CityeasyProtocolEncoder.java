...
public class CityeasyProtocolEncoder extends BaseProtocolEncoder {
  ...
  protected Object encodeCommand(Command command) {
    ...
    case Command.TYPE_POSITION_SINGLE:
      return encodeContent(CityeasyProtocolDecoder.MSG_LOCATION_REQUEST,
          content);
    case Command.TYPE_POSITION_PERIODIC:
      content.writeShort(((Number) command.getAttributes().get(
          Command.KEY_FREQUENCY)).intValue());
      return encodeContent(CityeasyProtocolDecoder.MSG_LOCATION_INTERVAL,
          content);
    ...
    case Command.TYPE_SET_TIMEZONE:
      int timezone = ((Number) command.getAttributes()
          .get(Command.KEY_TIMEZONE)).intValue();
      if (timezone < 0) {
        content.writeByte(1);
      } else {
        content.writeByte(0);
