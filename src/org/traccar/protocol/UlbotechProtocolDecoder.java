...
public class UlbotechProtocolDecoder extends BaseProtocolDecoder {
  ...
  public UlbotechProtocolDecoder(UlbotechProtocol protocol) {
    super(protocol);
    timeZone = Context.getConfig().getInteger(getProtocolName() + ".timezone",
        0);
  }
  ...
  private void decodeObd(Position position, ChannelBuffer buf, short length) {
    ...
    while (buf.readerIndex() < end) {
      ...
      position.add(ObdDecoder.decode(mode,
          ChannelBuffers.hexDump(buf.readBytes(parameterLength - 1))));
    }
  }
    ...
    while (buf.readableBytes() > 3) {
      ...
      case DATA_ADC:
        for (int i = 0; i < length / 2; i++) {
          ...
          position.set(Position.PREFIX_ADC + BitUtil.from(value, 12),
              BitUtil.to(value, 12));
        }
        break;
      ...
      case DATA_VIN:
        position.set(Position.KEY_VIN,
            buf.readBytes(length).toString(StandardCharsets.US_ASCII));
        break;
