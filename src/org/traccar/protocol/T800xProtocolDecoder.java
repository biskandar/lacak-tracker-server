...
public class T800xProtocolDecoder extends BaseProtocolDecoder {
  ...
  private static float readSwappedFloat(ChannelBuffer buf) {
    ...
    buf.readBytes(bytes);
    return ChannelBuffers.wrappedBuffer(ByteOrder.LITTLE_ENDIAN, bytes)
        .readFloat();
  }
  ...
  protected Object decode(Channel channel, SocketAddress remoteAddress,
    ...
    if (!identify(ChannelBuffers.hexDump(imei).substring(1), channel,
        remoteAddress)) {
      return null;
    }
    ...
    if (type == MSG_GPS || type == MSG_ALARM) {
      ...
      if (BitUtil.check(locationStatus, 6)) {
        ...
        position.setSpeed(UnitsConverter.knotsFromKph(BcdUtil.readInteger(buf,
            4) * 0.1));
        ...
        buf.readBytes(array);
        ChannelBuffer swapped = ChannelBuffers.wrappedBuffer(
