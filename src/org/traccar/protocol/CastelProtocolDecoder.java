...
public class CastelProtocolDecoder extends BaseProtocolDecoder {
  ...
  private void sendResponse(Channel channel, SocketAddress remoteAddress,
    ...
    if (channel != null) {
      ...
      ChannelBuffer response = ChannelBuffers.directBuffer(
          ByteOrder.LITTLE_ENDIAN, length);
      response.writeByte('@');
      response.writeByte('@');
      ...
      response.writeShort(Checksum.crc16(Checksum.CRC16_X25,
          response.toByteBuffer(0, response.writerIndex())));
  ...
  protected Object decode(Channel channel, SocketAddress remoteAddress,
    ...
    if (!identify(id.toString(StandardCharsets.US_ASCII).trim(), channel,
        remoteAddress)) {
      return null;
    }
    ...
    if (version == -1) {
      ...
      if (type == MSG_SC_HEARTBEAT) {
        ...
        sendResponse(channel, remoteAddress, version, id,
            MSG_SC_HEARTBEAT_RESPONSE, null);
        
      } else if (type == MSG_SC_LOGIN || type == MSG_SC_LOGOUT
          || type == MSG_SC_GPS || type == MSG_SC_ALARM
          || type == MSG_SC_CURRENT_LOCATION) {
        ...
        if (type == MSG_SC_LOGIN) {
          ...
          sendResponse(channel, remoteAddress, version, id,
              MSG_SC_LOGIN_RESPONSE, response);
        }
      ...
      if (type == MSG_CC_HEARTBEAT) {
        ...
        sendResponse(channel, remoteAddress, version, id,
            MSG_CC_HEARTBEAT_RESPONSE, null);
