...
public class AstraProtocolDecoder extends BaseProtocolDecoder {
  ...
  protected Object decode(Channel channel, SocketAddress remoteAddress,
    ...
    if (channel != null) {
      channel.write(ChannelBuffers.wrappedBuffer(new byte[] { 0x06 }),
          remoteAddress);
    }
    ...
    while (buf.readableBytes() > 2) {
      ...
      position
          .setSpeed(UnitsConverter.knotsFromKph(buf.readUnsignedByte() * 2));
      ...
      if (BitUtil.check(reason, 6) || BitUtil.check(reason, 7)) {
        ...
        position.set(Position.KEY_RFID,
            buf.readBytes(7).toString(StandardCharsets.US_ASCII));
