...
public class GlobalSatProtocolDecoder extends BaseProtocolDecoder {
  ...
  public GlobalSatProtocolDecoder(GlobalSatProtocol protocol) {
    ...
    format0 = Context.getConfig().getString(getProtocolName() + ".format0",
        "TSPRXAB27GHKLMnaicz*U!");
    format1 = Context.getConfig().getString(getProtocolName() + ".format1",
        "SARY*U!");
  }
  ...
  private Position decodeOriginal(Channel channel, SocketAddress remoteAddress,
      String sentence) {
    ...
    if (!format.contains("B")
        || !format.contains("S")
        || !(format.contains("1") || format.contains("2") || format
            .contains("3"))
        || !(format.contains("6") || format.contains("7") || format
            .contains("8"))) {
      return null;
    }
    ...
    for (int formatIndex = 0, valueIndex = 1; formatIndex < format.length()
      ...
      case 'B':
        ...
        dateBuilder.setHour(Integer.parseInt(value.substring(0, 2)))
      ...
      case 'I':
        position
            .setSpeed(UnitsConverter.knotsFromKph(Double.parseDouble(value)));
        break;
      case 'J':
        position
            .setSpeed(UnitsConverter.knotsFromMph(Double.parseDouble(value)));
        break;
      case 'K':
  ...
  private static final Pattern PATTERN = new PatternBuilder().text("$")
      ...
      .expression("([EW])").number("(ddd)(dd.d+),") // longitude (dddmm.mmmm)
      .expression("([NS])").number("(dd)(dd.d+),") // latitude (ddmm.mmmm)
      .number("(d+.?d*),") // altitude
      .number("(d+.?d*),") // speed
      .number("(d+.?d*)?,") // course
      .number("(d+)[,*]") // satellites
  ...
  private Position decodeAlternative(Channel channel,
      SocketAddress remoteAddress, String sentence) {
