...
public class MeitrackProtocolDecoder extends BaseProtocolDecoder {
  ...
  private static final Pattern PATTERN = new PatternBuilder().text("$$")
      ...
      .number("(?:,(x{6}(?:|x{6})*))?")
      // temperature
      .or().any().groupEnd().text("*").number("xx").text("\r\n").optional()
      .compile();
  
  private Position decodeRegularMessage(Channel channel,
      SocketAddress remoteAddress, ChannelBuffer buf) {
  ...
  private List<Position> decodeBinaryMessage(Channel channel,
      SocketAddress remoteAddress, ChannelBuffer buf) {
    ...
    while (buf.readableBytes() >= 0x34) {
      ...
      position.setTime(new Date((946684800 + buf.readUnsignedInt()) * 1000)); // 946684800
                                                                              // =
                                                                              // 2000-01-01
  ...
  protected Object decode(Channel channel, SocketAddress remoteAddress,
    ...
    case "D03":
      if (channel != null) {
        String imei = Context.getIdentityManager().getDeviceById(getDeviceId())
            .getUniqueId();
