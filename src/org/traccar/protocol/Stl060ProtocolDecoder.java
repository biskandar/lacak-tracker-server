...
public class Stl060ProtocolDecoder extends BaseProtocolDecoder {
  ...
  private static final Pattern PATTERN = new PatternBuilder().any().text("$1,")
      .number("(d+),") // imei
      .text("D001,") // type
      .expression("[^,]*,") // vehicle
      ...
      .number("(d+.?d*),") // speed
      .number("(d+.?d*),") // course
      .groupBegin().number("(d+),") // odometer
      .number("(d+),") // Ignition
      .number("(d+),") // di1
      .number("(d+),") // di2
      .number("(d+),") // fuel
      .or().expression("([01]),") // charging
      ...
      .expression("([01]),") // do1
      .expression("([01]),") // do2
      .groupEnd().expression("([AV])") // validity
      .any().compile();
  ...
  protected Object decode(Channel channel, SocketAddress remoteAddress,
    ...
    if (parser.hasNext(5)) {
      ...
      position
          .set(Position.KEY_INPUT, parser.nextInt() + parser.nextInt() << 1);
    ...
    if (parser.hasNext(10)) {
      ...
      position.set(Position.KEY_OUTPUT,
          parser.nextInt() + parser.nextInt() << 1);
    }
