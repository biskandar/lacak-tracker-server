...
public class PathAwayProtocolDecoder extends BaseProtocolDecoder {
  ...
  private static final Pattern PATTERN = new PatternBuilder().text("$PWS,")
      .number("d+,") // version
      .expression("[^,]*,") // name
      .expression("[^,]*,") // icon
      .expression("[^,]*,") // color
  ...
  protected Object decode(Channel channel, SocketAddress remoteAddress,
    ...
    if (!identify(decoder.getParameters().get("UserName").get(0), channel,
        remoteAddress)) {
      return null;
    }
    
    Parser parser = new Parser(PATTERN, decoder.getParameters().get("LOC")
        .get(0));
    if (!parser.matches()) {
      return null;
    }
